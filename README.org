...
#+BEGIN_SRC elisp :tangle
  ...
  (defun to-seconds (minutes)
    "Converts the given minutes to seconds"
    (* minutes 60))

  (defun start-pomodoro-timer (&optional message)
    (interactive)
    (when (yes-or-no-p (or message
                           "Would you like to start a pomodoro session?"))
      (run-at-time (to-seconds 1)
                   nil
                   (lambda ()
                     (let ((suggsted-activity (first
                                               (seq-filter (lambda (item)
                                                             (not
                                                              (member item (or pomodoro--completed-activities
                                                                               '()))))
                                                           pomodoro-activity-list)))))
                     (when (not pomodoro--completed-activities))
                     (message "Time to take a break!")
                     (alert (format "Get up and %s" suggested-activity))
                     (switch-to-buffer "*Messages*"
                                       nil
                                       't)
                     (pomodoro-break (setq pomorodo--round (+1 pomodoro--round)))))))

  (defun pomodoro-break (multiplier)
    (run-at-time (to-seconds (* multiplier 5))
                 nil
