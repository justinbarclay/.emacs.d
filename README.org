...
- [[#package-manager][Package Manager]]
  ...
  - [[#post-bootstrap-integration-with-packageel][post bootstrap integration with package.el]]
  - [[#enable-garbage-collection-stats-tracking][Enable garbage collection stats tracking]]
- [[#tent-pole][Tent Pole]]
...
** post bootstrap integration with package.el
Now that we've installed elpaca we can initialize our packages so normal use-package can work.
#+begin_src emacs-lisp :tangle no
(package-initialize)
#+end_src
** Enable garbage collection stats tracking
Now that we have elpaca installed we can set up our GC tracking.
#+begin_src emacs-lisp
...
#+RESULTS:
: #s(hash-table size 65 test eql rehash-size 1.5 rehash-threshold 0.8125 data (:use-package (25800 5180 218838 0) :use-package-secs (0 0 2536 0) :init (25800 5140 546343 0) :init-secs (0 0 15 0) :config (25800 5140 546338 0) :config-secs (0 0 2 0)))
