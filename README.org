...
- [[#setup][Setup]]
  ...
  - [[#lsp-mode-optimizations][Lsp mode optimizations]]
- [[#vars][Vars]]
  - [[#os][OS]]
- [[#tent-pole][Tent Pole]]
...
- [[#languages][Languages]]
  ...
  - [[#llm--agentic-flows][LLM & Agentic Flows]]
...
- [[#custom-functions][Custom Functions]]
  ...
  - [[#xstate-dynamic-params][xstate dynamic params]]
...
This sets the garbage collection threshold to 200mb
...
Other dependencies need updated packages.

This was occasionally needed by Elpaca because Elpaca decided not to handle this. So we handled it for them, but this also should help when we manually upgrade jsonrpc
#+begin_src elisp :tangle early-init.el
...
Modern research workflow using =org-cite=, =citar=, and =biblio.el=.

**** citar
=citar= is a modern bibliography frontend for Emacs. It provides a fast and feature-rich interface to your BibTeX files, integrating perfectly with =vertico=, =consult=, and =embark=.
#+begin_src elisp
  (use-package citar
    :after org
    :custom
...
**** citar-org-roam
Integrate =citar= with =org-roam= for a seamless Zettelkasten research workflow.
...
**** biblio
Search and import bibliographic entries from various sources (ArXiv, DOI, Crossref, etc.) directly into your BibTeX file.
...
**** citeproc
The =citeproc= package is required for exporting citations using the Citation Style Language (CSL).
...
#+RESULTS:
: #s(hash-table data (:use-package (27161 53088 546714 931000) :use-package-secs (0 0 168 588000) :init (27161 53088 546709 651000) :init-secs (0 0 42 739000) :config (27161 53088 546695 651000) :config-secs (0 0 26 880000)))
...
*** Diff Highlights
Show per-line git diff indicators in the fringe — added, changed, and deleted lines.
#+begin_src elisp
  (use-package diff-hl
...
*** Reversible frames
In the same spirit as the above, Emacs 31 can resurrect a deleted frame with ~C-x 5 u~.
#+begin_src elisp
  (undelete-frame-mode 1)
#+end_src

*** Quitting windows
Make ~q~ kill the buffer rather than bury it, and have ~kill-buffer~ tidy up the window it
was displayed in instead of leaving an orphan.
#+begin_src elisp
...
** Performance
Increase the size of read-process-output-max from default of 4k to 4Mb
#+BEGIN_src elisp :tangle early-init.el
  (setq-default read-process-output-max (* 4 1024 1024)) ;; 4mb
#+END_SRC

Skip fontification during input for smoother typing and scrolling
#+BEGIN_src elisp :tangle early-init.el
  (setopt redisplay-skip-fontification-on-input t)
#+END_SRC

Disable cursors in non-focused windows
#+BEGIN_src elisp :tangle early-init.el
...
- Compacting font caches causes visible stutters on Windows, especially
  with Nerd Fonts and emoji fontsets. Trade a bit of memory to avoid it.
- ~w32-get-true-file-attributes~ makes every file operation issue extra
  syscalls to resolve hard-link counts and ACLs; disabling it speeds up
  dired, recentf, and anything else that stats lots of files.
#+BEGIN_src elisp
  (when jb/os-windows-p
...
#+END_SRC
** Autorevert
Automatically refresh buffers when the underlying file changes on disk
#+BEGIN_src elisp
...
#+end_src
*** Anzu
Show match count and position during isearch and query-replace
#+begin_src elisp
  (use-package anzu
    :config
    (global-anzu-mode))
...
*** Wgrep
Make grep/rg/consult-grep results directly editable in-buffer — edit and save to apply changes across files.
#+begin_src elisp
  (use-package wgrep
    :custom
...
A few Emacs 31 conveniences: ~C-w~ with no active region kills the previous word instead of
erroring, ~delete-pair~ leaves a mark behind so we can get back, and yanked text gets reindented.
#+begin_src elisp
...
Emacs 31 lets us stop matching parens that only appear inside comments and strings. ~all~
suppresses them entirely; ~on-mismatch~ is the softer option, which only skips the mismatch
highlighting.
#+BEGIN_src elisp
  (show-paren-mode 1)
...
** Whitespace
Emacs doesn’t handle trailing spaces or anything like that very well by default, it’s far too aggressive for my tastes, so we’ll use stripspace to fix this.
...
Superseded by built-in hideshow below, but kept untangled in case we want to switch back.
#+begin_src elisp :tangle no
...
As of Emacs 31, ~hideshow~ folds over tree-sitter nodes ­— ~hs-treesit-things~ defaults to the
~list~ thing — which is what we were pulling in ~treesit-fold~ for. It also covers the two
settings we had customized, and adds fringe indicators and state cycling that ~treesit-fold~
does not have:

| ~treesit-fold-line-count-format~ | ~hs-display-lines-hidden~ |
| ~treesit-fold-on-next-line~      | ~hs-hide-block-behavior~  |
| ~treesit-fold-toggle~            | ~hs-cycle~                |

Worth checking fold granularity on a TSX file: the custom ~jsx_element~ rules above exist
because ~treesit-fold~ got those ranges wrong, and whether hideshow's ~list~ navigation does
better is an open question. If it doesn't, flip the tangle flags back around.
#+begin_src elisp
...
** Tree Sitter support
As of Emacs 31, ~treesit~ handles on its own what ~treesit-auto~ used to do for us. Setting
~treesit-enabled-modes~ to ~t~ populates ~major-mode-remap-alist~ with every bundled ts-mode, and
~treesit-auto-install-grammar~ offers to fetch a missing grammar when we first open such a file.

Every grammar the bundled modes ask for already has a recipe: each mode file registers its own
entry in ~treesit-language-source-alist~ when it loads, pinned to a tested commit. That covers all
28 grammars the 27 remapped modes request — including ~javascript~ and ~jsdoc~, which is why we no
longer hand-roll those recipes.

Two of our third-party modes are not covered, so we map them by hand:
- ~kotlin-ts-mode~ registers no ~auto-mode-alist~ entry at all.
- ~nushell-ts-mode~ registers one, but guards it behind a load-time ~(treesit-ready-p 'nu)~, which
  can never succeed before the grammar exists.

Both gate on ~treesit-ready-p~ rather than ~treesit-ensure-installed~, so they will not auto-install.
Run ~M-x treesit-install-language-grammar~ once for =kotlin= and =nu=; the recipes below are what
makes that possible.
#+begin_src elisp
  (use-feature treesit
    :custom
...
#+end_src
**** LLM
LLM is a library that provides a unified interface for interacting with different Large Language Model providers. It is the powerhouse that powers Ellama.
...
#+end_src
**** Ellama
Ellama is the primary interface for LLM interactions, leveraging the =llm= abstraction layer for multiple backends, =vecdb= for local RAG, and =mcp= for environment interaction.

Commit message generation runs on ~git-commit-setup-hook~ at depth 90 rather than the default,
and forces point to ~point-min~ first. Both are load-bearing. ~ellama--insert~ calls a bare,
unbounded ~fill-paragraph~ after every streamed chunk whenever the major mode derives from
~text-mode~ — which the commit buffer does — and only ellama's own text carries the ~hard~
newline property that survives filling. The single blank line ~git-commit-ensure-comment-gap~
leaves above the ~#~ block is therefore the only thing keeping the template and the verbose diff
out of the same paragraph as the message.

At the default depth ellama runs /before/ that gap exists, so the insertion marker (type ~t~,
created synchronously at position 1) gets pushed to position 2 when the gap opens, and the reply
is written into the gap instead of above it. The message ends up glued to ~# Please enter…~ and
the fill flattens the rest of the buffer into one wrapped blob.

If this regresses, ~(setq-local ellama-fill-paragraphs nil)~ in the wrapper disables the fill
outright — a buffer-local set, not a ~let~, since ellama reads it from the async callback long
after the hook returns. The cost is an unwrapped message body.
...
#+end_src
** xstate dynamic params
Converts a normal xstate action to one with dynamic params. This is really useful when you want to have a single action that can be used across multiple states but with different parameters.
Example:
#+begin_src js :tangle no
  ...
  {
    //...
    actions: { type: "someAction", params: ({event}) => event.output }
    //...
  }
#+end_src
